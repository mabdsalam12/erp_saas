<?php
    ...
    if($rType==0){
        ...
        if(!empty($purchase) && !empty($purchaseDetails)){  
            ...
            foreach($purchase as $p){
                if(!isset($supplierData[$p['createdBy']])){
                    $supplierData[$p['createdBy']]=[
                        'amount'=>0,
                        'qty'=>0
                    ];
                }
                $supplierData[$p['createdBy']]['amount']+=$p['netTotal'];
                $supplierData[$p['createdBy']]['qty']+=$quantitys[$pd['purID']];
            }
            foreach($supplierData as $uID=>$sd){
                $tQty+=$sd['qty'];
                $total+=$sd['amount'];
                $rData[]=[
                    's'=>$sr++,
                    'u'=>$users[$uID]['username'],
    ...
    elseif($rType==1){
        ...
        if(!empty($purchase) && !empty($purchaseDetails)){  
            foreach($purchaseDetails as $pd){
                ...
                if(!isset($prodcutData[$uID][$pd['pID']])){
                    $prodcutData[$uID][$pd['pID']]=[
                        'amount'=>0,
                        'qty'=>0,
                    ];
                }
                $prodcutData[$uID][$pd['pID']]['amount']+=$pd['quantity']*$pd['unitPrice'];
                $prodcutData[$uID][$pd['pID']]['qty']+=$pd['quantity'];
            }
            ...
            foreach($prodcutData as $uID=>$data){
        ...
        if(!empty($purchase) && !empty($purchaseDetails)){  
            foreach($purchaseDetails as $pd){
                ...
                if(!isset($prodcutData[$date][$supID][$uID][$pd['pID']])){
                    $prodcutData[$date][$uID][$supID][$pd['pID']]=[
                        'amount'=>0,
                        'qty'=>0,
                        'uID'=>$supID,
                    ];
                }
                $prodcutData[$date][$uID][$supID][$pd['pID']]['amount']+=$pd['quantity']*$pd['unitPrice'];
                $prodcutData[$date][$uID][$supID][$pd['pID']]['qty']+=$pd['quantity'];
            }
            $products = $db->selectAll('products','where pID in('.implode(',',$pIDs).')','pID,pTitle,categoryID');
            $general->arrayIndexChange($products,'pID');
            ...
            foreach($prodcutData as $date=>$datass){
                foreach($datass as $uID=>$datas){
    ...
    $report_data=array(
        ...
        'title'     => 'Usr wise Purchase Report',
        'info'      => $reportInfo,
        'fileName'  => $fileName,
        'head'=>$head,
        'data'=>$rData
