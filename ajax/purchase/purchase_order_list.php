<?php
    ...
    $order = $db->selectAll('purchase_order','where date  between '.$from.' and '.$to);
    $rData=[];
    $sr=1;
    if(!empty($order)){
        $user_ids=[];
        ...
        foreach($order as $o){
            $rData[]=[
                ...
                'action'=>'<button onclick="purchase_order_details_view('.$o['id'].')" class="btn btn-success">Details</button>',
            ];
        }
    }
    $fileName='purchase_order_list'.TIME.rand(0,999).'.txt';
    $report_data=array(
        ...
        'head'=>array(
            ...
            array('title'=>"Details"    ,'key'=>'action'),
        ),
        'data'=>$rData
    );
