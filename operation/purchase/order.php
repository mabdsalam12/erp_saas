...
<?php 
    ...
    <div class="col-sm-12">
        <div class="white-box border-box">
            ...
            <div class="row">
                <div class="col-sm-12">
                    <button onclick="purchase_order_add_to_cart()" class="m-2 btn btn-info waves-effect waves-light pull-right m-t-10">Add to cart</button>
                </div>
            </div>
            <div class="row">
                <div style="display: none;">
                    <table>
                        <tbody>
                            <tr id="purchase-order-details-tr">
                        ...
                        <tfoot>
                            ...
                            <tr>
                                <td colspan="4" class="amount_td">
                                    ...
                                    <button onclick="purchase_order_add()" class="m-1 p-1 btn btn-lg btn-success pull-right">Submit</button>
    ...
    <script>
        ...
        function purchase_order_add_to_cart(){
            ...
            if(errorSet==0){
                ...
                $('#purchase-order-details-tr .product-id').val(product_id)

                $('#purchase-order-details-tr .product-title').html(product.t)
                $('#purchase-order-details-tr .unit-title').html(product.u)

                $('#purchase-order-details-tr .quantity').html(quantity)
                $('#purchase-order-details-tr .remove').attr('onclick','remove_row_by_id(\''+id+'\');')
                let purchase_order_details=$('#purchase-order-details-tr').html();
                $('#purchase-order-details').append('<tr id="'+id+'">'+purchase_order_details+'</tr>');
                $('#product-id').val('');
                $('#quantity').val('');
                select2Call();
                let tr_sl_start=1;$('#purchase-order-details .autoSerial').each(function(){$(this).html(tr_sl_start);tr_sl_start++;});
            }
        }
        function purchase_order_add(){
            ...
            $('#purchase-order-details .product-id').each(function(a,b){
                if(errorSet==0){
                ...
                <div class="row">
                    <div class="col-sm-12 col-lg-12">   
                        ...
                        <div class="col-md-2">
                            ...
                            <button class="btn btn-success" onclick="purchase_order_list()">Search</button>
                            <script type="text/javascript">
                                function purchase_order_list(){
                                    ...
                                    $.ajax({
                                        type:'post',
                                        url:ajUrl,
                                        data:{purchase_order_list:1,dRange:dRange},
                                        success:function(data){
                                            $('#reportArea').html('');
                                            if(typeof(data.status)!=='undefined'){
                                                if(data.status==1){
                                ...
                                function purchase_order_details_view(id){
                                    $('#details-body').html(loadingImage);
                                    $.ajax({
                                        type:'post',
                                        url:ajUrl,
                                        data:{purchase_order_details_view:1,id:id},
                                        success:function(data){
                                            $('#details-body').html('');
                                            if(typeof(data.status)!=='undefined'){
                                                if(data.status==1){
