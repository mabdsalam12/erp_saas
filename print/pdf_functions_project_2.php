...
class MY_PDF extends TCPDF {
    ...
    public function pdf_init($data,General $general){
        ...
        $this->SetAlpha(0.1);
        $this->Image(ROOT_DIR.'/images/'.PROJECT.'/logo_big.png', 40, 85, 130 );
        $this->SetAlpha(1);
        $this->data=$data;
    }
    ...
    public function sale_product($y,$with_tp=1) {
        ...
        $x=$this->left_margin;
        $h=6;$w1=8;$w3=15;$w4=15;$w5=17;$w6=16;
        $w7=18;$w8=22;$w9=20;$w10=15;
        $table_width=$w1+$w4+$w4+$w5+$w6+$w7+$w8+$w9+$w10;
    ...
    //    echo $pgw.' '.$table_width.' '.$margin.' '.$w3;exit; 
        ...
        $x=$this->createTableHeaderCell($w10,$h,'SP',$x,$y);
       ...
       // $y+=1;
        ...
        foreach($this->data['details'] as $d){
            ...
            $box_height=$h;
            // $box_height=6;
            $pText=$d['product']['title'];
            if($this->getStringHeight($w2,$pText)>$box_height){
                $box_height=$this->getStringHeight($w2,$pText);
                $h=$box_height+2;
            }

            $p= $d['product'];
            $x=$this->left_margin;
            $unit=$this->data['units'][$p['unit_id']]['title'];
            $sale_price=round($d['total']/$d['sale_qty'],2);
            $this->SetFont($this->font_name,'',8);
            $x=$this->createTableBodyCell($w1,$h,$serial++,$x,$y);
            $x=$this->createTableBodyCell($w2,$h,$pText,$x,$y);
            $x=$this->createTableBodyCell($w3,$h,$unit,$x,$y);
            $x=$this->createTableBodyCell($w4,$h,(float)$d['unit_price'],$x,$y,'R');
            $x=$this->createTableBodyCell($w10,$h,$sale_price,$x,$y,'R');
        ...
        $x=$this->createTableHeaderCell($w1+$w2+$w3+$w4+$w10+$w5+$w6+$w7,$box_height,'Total',$x,$y,'R');
        ...
        //TAKA IN WORD
        $this->MultiCell(100,0,'TAKA IN WORD: '.ucfirst($amount_in_word).' taka only.',0,'L',false,0,$x,$y);
